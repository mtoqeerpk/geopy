...
class plot2dseisxl(object):
    ...
    def setupGUI(self, Plot2DSeisXl):
        Plot2DSeisXl.setObjectName("Plot2DSeisXl")
        Plot2DSeisXl.setFixedSize(400, 450)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.join(self.iconpath, "icons/visxl.png")),
                       QtGui.QIcon.Normal,
                       QtGui.QIcon.Off)
        Plot2DSeisXl.setWindowIcon(icon)
        #
        self.lblattrib = QtWidgets.QLabel(Plot2DSeisXl)
        self.lblattrib.setObjectName("lblattrib")
        self.lblattrib.setGeometry(QtCore.QRect(10, 10, 150, 30))
        self.lwgattrib = QtWidgets.QListWidget(Plot2DSeisXl)
        ...
        self.lblslice = QtWidgets.QLabel(Plot2DSeisXl)
        self.lblslice.setObjectName("lblslice")
        self.lblslice.setGeometry(QtCore.QRect(10, 230, 150, 30))
        self.slbslice = QtWidgets.QScrollBar(Plot2DSeisXl)
        ...
        self.ldtslice = QtWidgets.QLineEdit(Plot2DSeisXl)
        ...
        self.ldtmax = QtWidgets.QLineEdit(Plot2DSeisXl)
        ...
        self.lblrangeto = QtWidgets.QLabel(Plot2DSeisXl)
        ...
        #
        self.btnconfigplayer = QtWidgets.QPushButton(Plot2DSeisXl)
        ...
        #
        self.btnplotslice = QtWidgets.QPushButton(Plot2DSeisXl)
        ...
        #
        self.msgbox = QtWidgets.QMessageBox(Plot2DSeisXl)
        self.msgbox.setObjectName("msgbox")
        _center_x = Plot2DSeisXl.geometry().center().x()
        _center_y = Plot2DSeisXl.geometry().center().y()
        self.msgbox.setGeometry(QtCore.QRect(_center_x - 150, _center_y - 50, 300, 100))
        #
        self.retranslateGUI(Plot2DSeisXl)
        QtCore.QMetaObject.connectSlotsByName(Plot2DSeisXl)



    def retranslateGUI(self, Plot2DSeisXl):
        self.dialog = Plot2DSeisXl
        #
        _translate = QtCore.QCoreApplication.translate
        Plot2DSeisXl.setWindowTitle(_translate("Plot2DSeisXl", "2D Window: Seismic Crossline"))
        self.lblattrib.setText(_translate("Plot2DSeisXl", "Select target properties:"))
        if (self.checkSurvInfo() is True) and (self.checkSeisData() is True):
            ...
            for i in sorted(self.seisdata.keys()):
                ...
                item.setText(_translate("Plot2DSeisXl", i))
        ...
        #
        self.lblslice.setText(_translate("Plot2DSeisXl", "Select crossline slice:"))
        ...
        if (self.checkSurvInfo() is True):
            self.ldtslice.setText(_translate("Plot2DSeisXl", str(_slicemin)))
        else:
            self.ldtslice.setText(_translate("Plot2DSeisXl", ''))
        self.slbslice.valueChanged.connect(self.changeSlbSlice)
        self.ldtslice.textChanged.connect(self.changeLdtSlice)
        #
        self.lblcmap.setText(_translate("Plot2DSeisXl", "\t  Color map:"))
        ...
        #
        self.cbxflip.setText(_translate("Plot2DSeisXl", ""))
        self.cbxflip.setIcon(QtGui.QIcon(
            QtGui.QPixmap(os.path.join(self.iconpath, "icons/flip.png")).scaled(80, 80)))
        #
        self.lblrange.setText(_translate("Plot2DSeisXl", "\t       Range:"))
        self.lblrangeto.setText(_translate("Plot2DSeisXl", "~~~"))
        if (self.checkSurvInfo() is True) \
            ...
            self.ldtmin.setText(_translate("Plot2DSeisXl", str(_min)))
            self.ldtmax.setText(_translate("Plot2DSeisXl", str(_max)))
        #
        self.btnconfigplayer.setText(_translate("Plot2DSeisXl", "Player Configuration"))
        self.btnconfigplayer.clicked.connect(self.clickBtnConfigPlayer)
        #
        self.btnplotslice.setText(_translate("Plot2DSeisXl", "Seismic Crossline Viewer"))
    ...
    def clickBtnPlotSlice(self):
        ...
        if len(_attriblist) < 1:
            print("Plot2DSeisXl: No property selected for plot")
        ...
        if _min is False or _max is False:
            print("Plot2DSeisXl: Non-float range specified for plot")
            QtWidgets.QMessageBox.critical(self.msgbox,
                                           '2D Window: Seismic Crossline',
        ...
        for i in range(len(_attriblist)):
            print("Plot2DSeisXl: Plot %d of %d properties: %s" % (i + 1, len(_attriblist), _attriblist[i].text()))
    ...
    def checkSeisData(self):
        ...
        for f in self.seisdata.keys():
            if np.shape(self.seisdata[f])[0] != self.survinfo['SampleNum']:
                # print("Plot2DSeisXl: Seismic & survey not match")
                # QtWidgets.QMessageBox.critical(self.msgbox,
                #                                'Plot Seismic Crossline',
...
if __name__ == "__main__":
    ...
    Plot2DSeisXl = QtWidgets.QWidget()
    gui = plot2dseisxl()
    gui.setupGUI(Plot2DSeisXl)
    Plot2DSeisXl.show()
    sys.exit(app.exec_())